		...
		private void Paint(Graphics graphics)
		{
			...
			foreach (string Key in graphs.Keys)
			{
				...
				for (int i = 1; i < graphValue.Count; i++)
				{
					...
					Point p1, p2, p1Bottom, p2Bottom;
					endPoint = graphValue[i];
					decimal displayValue1, displayValue2, displayPosition1, displayPosition2;
					displayValue1 = GetRelativePosition(startPoint.DisplayValue, minValue, maxValue);
					displayValue2 = GetRelativePosition(endPoint.DisplayValue, minValue, maxValue);
					displayPosition1 = GetRelativePosition(startPoint.DisplayPosition, minPosition, maxPosition);
					displayPosition2 = GetRelativePosition(endPoint.DisplayPosition, minPosition, maxPosition);
					p1 = new Point((int)(displayPosition1 * width), height - (int)(displayValue1 * height));
					p2 = new Point((int)(displayPosition2 * width), height - (int)(displayValue2 * height));
					if (p2.X - p1.X < 1)
					{
						//continue;
					}
					else
					{
						switch (visualizationMode)
						{
							case VisualizationMode.Lines:
								...
								{
									p1Bottom = new Point(p1.X, height);
									p2Bottom = new Point(p2.X, height);
									pArray[0] = p1;
									pArray[1] = p2;
									pArray[2] = p2Bottom;
									pArray[3] = p1Bottom;
